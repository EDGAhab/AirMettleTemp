...
if not os.path.isdir(audio_dir):
    ...
    trak_byte_range, video_trak_idx, audio_trak_idx, _ = finding_traks(
        moov_data, st_name)
    print("trak_byte_range")
    print(trak_byte_range)
    print("video_trak_idx")
    print(video_trak_idx)
    print("audio_trak_idx")
    print(audio_trak_idx)
    audio_table = []
    audio_stcz = []
    for i, byte_range in enumerate(trak_byte_range):
        ...
        if i in video_trak_idx:
            ...
            print(" i in audio ")
            audio_table = get_sample_table(
                moov_data[int(byte_range[0]): int(byte_range[1])], st_name)
            audio_stcz =get_bytes_of_chunks(
                audio_table[0], audio_table[1], len(audio_table[2])) 
    ...
    max_audio_stco=max([audio_table[2][j][0] for j in range(len(audio_stcz))])
    print("max_video_stco")
    print(max_video_stco)
    print("max_audio_stco")
    print(max_audio_stco)
    max_stco = max(max_audio_stco,max_video_stco)+1
    
    while flag:
        cuurent_chunk_offset = []
        current_ptr = audio_ptr
        cuurent_chunk_offset.append(audio_table[2][current_ptr][0])
        cuurent_chunk_offset.append(video_table[2][video_ptr][0])
        chunk_offset_argsort = argsort(cuurent_chunk_offset)
        print(chunk_offset_argsort)
        select_trakid = chunk_offset_argsort[0]
        print("select_trakid: ", select_trakid)
        if select_trakid == 1:
            print("in video")
            with open(recon_file, 'ab') as fout:
            ...
            print("in audio aaaaa")
            with open(recon_file, 'ab') as fout:
                    fout.write(binascii.unhexlify(
                        audio_mdat[
                            audio_mdat_offset: audio_mdat_offset+audio_stcz[audio_ptr]*2]
                            ))
        ...
        if video_ptr == len(video_stcz) and audio_ptr == len(audio_stcz):
            flag = False
